    ...
    def fill_attendance():
        ...
        if subb=='':
            ...
            def create_csv():
                ...
                connection.commit()
                csv_name='D:\project microsoft\Attendace_management_system-master\Attendace_management_system-master\Attendance'+DB_table_name+'.csv'
                with open(csv_name, "w") as csv_file:
                    csv_writer = csv.writer(csv_file)
                    csv_writer.writerow([i[0] for i in cursor.description])  # write headers
                    csv_writer.writerows(cursor)
                    O="CSV created Successfully"
                    Notifi.configure(text=O, bg="Green", fg="white", width=33, font=('times', 19, 'italic'))
                ...
                with open(csv_name, newline="") as file:
                    ...
                    for col in reader:
                        ...
                        for row in col:
                            ...
                            label = tkinter.Label(root, width=13, height=1, fg="black", font=('times', 13, 'italic'),
            ...
            def attf():
                import subprocess
                subprocess.Popen(r'explorer /select,"D:\project microsoft\Attendace_management_system-master\Attendace_management_system-master\Attendance\Manually Attendance/-------Check atttendance-------"')

            attf = tk.Button(MFW,  text="Check Sheets",command=attf,fg="black"  ,bg="Red"  ,width=12  ,height=1 ,activebackground = "lawn green" ,font=('times', 14, 'italic'))
            attf.place(x=730, y=410)

            MFW.mainloop()


    SUB = tk.Label(sb, text="Enter Subject", width=15, height=2, fg="white", bg="blue2", font=('times', 15, 'italic '))
    SUB.place(x=30, y=100)

    global SUB_ENTRY

    SUB_ENTRY = tk.Entry(sb, width=20, bg="yellow", fg="red", font=('times', 23, ' italic'))
...
def err_screen():
    ...
    Label(sc1,text='Enrollment & Name required!!!',fg='red',bg='white',font=('times', 16, ' italic')).pack()
    Button(sc1,text='OK',command=del_sc1,fg="black"  ,bg="lawn green"  ,width=9  ,height=1, activebackground = "Red" ,font=('times', 15, ' italic')).place(x=90,y= 50)
...
def err_screen1():
    ...
    Label(sc2,text='Please enter your subject name!!!',fg='red',bg='white',font=('times', 16, 'italic ')).pack()
    Button(sc2,text='OK',command=del_sc2,fg="black"  ,bg="lawn green"  ,width=9  ,height=1, activebackground = "Red" ,font=('times', 15, ' italic ')).place(x=90,y= 50)
...
def take_img():
    ...
    if l1 == '':
        ...
        try:
            ...
            Notification.configure(text=res, bg="SpringGreen3", width=50, font=('times', 18, 'italic'))
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                Notifica.configure(text=M, bg="Green", fg="white", width=33, font=('times', 15, 'italic'))
                Notifica.place(x=20, y=250)
                ...
                root.configure(background='snow')
                cs = 'D:/project microsoft/Attendace_management_system-master/Attendace_management_system-master/' + fileName
                with open(cs, newline="") as file:
                    ...
                    for col in reader:
                        c = 0
                        for row in col:
                            # In this piece of code i have added some styling
    ...
    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"D:/project microsoft/Attendace_management_system-master/Attendace_management_system-master/Attendance/-------Check atttendance-------"')

    attf = tk.Button(windo,  text="Check Sheets",command=Attf,fg="black"  ,bg="lawn green"  ,width=12  ,height=1 ,activebackground = "Red" ,font=('times', 14, 'italic '))
    attf.place(x=430, y=255)

    sub = tk.Label(windo, text="Enter Subject", width=15, height=2, fg="white", bg="blue2", font=('times', 15, ' italic '))
    sub.place(x=30, y=100)

    tx = tk.Entry(windo, width=20, bg="yellow", fg="red", font=('times', 23, ' italic '))
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == 'Lohitha003' :
            if password == 'demo123':
                ...
                cs = 'D:/project microsoft/Attendace_management_system-master/Attendace_management_system-master/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
                    ...
                    for col in reader:
                        ...
                        for row in col:
                            #IN this piece of code i have added some styling
                            label = tkinter.Label(root, width=8, height=1, fg="black", font=('times', 15, 'italic '),
                                                  bg="lawn green", text=row, relief=tkinter.RIDGE)
                            label.grid(row=r, column=c)
                            c += 1
                        r += 1
                ...
                Nt.configure(text=valid, bg="red", fg="black", width=38, font=('times', 19, 'italic'))
    ...
    Nt = tk.Label(win, text="Attendance filled Successfully", bg="yellow", fg="white", width=40,
                  height=2, font=('times', 19, 'italic'))
    # Nt.place(x=120, y=350)
    ...
    un_entr = tk.Entry(win, width=20, bg="yellow", fg="red", font=('times', 23, ' italic'))
    un_entr.place(x=290, y=55)

    def c11():
        pw_entr.delete(first=0, last=22)

    pw_entr = tk.Entry(win, width=20,show="*", bg="yellow", fg="red", font=('times', 23, ' italic '))
    ...
    Login = tk.Button(win, text="LogIn", fg="black", bg="lime green", width=20,
                       height=2,
                       activebackground="Red",command=log_in, font=('times', 15, ' italic '))
...
def trainimg():
    ...
    try:
        ...
        Notification.configure(text=q, bg="SpringGreen3", width=50, font=('times', 18, 'italic'))
        Notification.place(x=350, y=400)
...
txt = tk.Entry(window, validate="key", width=20, bg="yellow", fg="red", font=('times', 25, ' italic '))
